...
const SubCategories = () => {
    ...
    const getAllAdvertisement = async () => {
        try {
            const response = await axios.get(`https://api.assetorix.com:5500/adifie/listing/subcategory/${selectedId}/advertisement`)
                ...
                <div className="advertisememnt sm:w-[80vw] w-auto mx-auto sm:mt-10 mt-0">
                    ...
                    <div className="sub-header flex sm:flex-row flex-col mb-10 rounded-xl">
                        <div className="left sm:w-[60vw] w-[80vw] mx-auto">
                            ...
                            <p className='font-bold text-3xl text-center mt-10'>Advertisement Types</p>

                            <div className='grid grid-cols-1 mb-10'>
                                {
                                    getAds?.map((ad) => {
                                        const currencySymbol = ad.currency === 'INR' ? '₹' : ad.currency === 'EUR' ? '€' : ad.currency === 'USD' ? '$' : ''
                                        { console.log(ad.currency) }
                                        return (
                                            <>
                                                <div className="top-choice flex flex-col gap-3 relative border right-10 p-10">
                                                    {/* <div
                                        onClick={() => deleteAdvertisement(ad._id)}
                                        className='h-[50px] w-[50px] rounded-full bg-red-500 flex justify-center items-center cursor-pointer'>
                                        <MdDelete className='text-2xl text-white' />
                                    </div> */}

                                                    <div className="image flex justify-center">
                                                        <div className='sm:w-[100%] justify-center items-center p-5'>
                                                            {!loading &&
                                                                <Slider images={ad?.image} />}
                                                        </div>
                                                    </div>
                                                    <div className="ribbon-2 absolute text-white font-bold">POPULAR</div>
                                                    <p className='font-bold text-[#2f2c44] text-xl'>{ad.fullTitle}</p>
                                                    <p className='font-semibold text-[#2f2c44] text-sm'>{ad.shortDescription}</p>
                                                    ...
                                                    <div className='flex justify-between'>
                                                        ...
                                                        <div>
                                                            <p className='font-semibold'>{currencySymbol} {ad?.price} {ad?.pricePostFix} </p>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div className='flex justify-between'>
                                                        <div className='flex gap-2 border border-[#4e3bc9] p-3 h-[40px] rounded-lg'>
                                                            ...
                                                            <button
                                                                onClick={() => navigate(`/subpages/${ids}/subcategory/${selectedId}/${ad._id}`)}
